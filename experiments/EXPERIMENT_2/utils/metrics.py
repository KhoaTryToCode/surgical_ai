...
def evaluate_batch(pred_logits, gt_masks):
    ...
    for b in range(pred_classes.shape[0]):
        ...
        for c in range(1, 4):
            p_bin = (p_map == c).astype(np.uint8)
            g_bin = (g_map == c).astype(np.uint8)

            d, iou = compute_dice_iou(p_bin, g_bin)
            assd = compute_assd(p_bin, g_bin)

            class_dices.append(d)
            class_ious.append(iou)
            class_assds.append(assd)

        # Macro average over 3 foreground landmark classes
        macro_dice = float(np.mean(class_dices))
        macro_iou = float(np.mean(class_ious))
        macro_assd = float(np.mean(class_assds))

        # Whole foreground union metrics
        p_fg = (p_map > 0).astype(np.uint8)
        g_fg = (g_map > 0).astype(np.uint8)
        ...
        batch_metrics.append({
            ...
            'ridge_assd': class_assds[0],
            'sil_assd': class_assds[1],
            'falc_assd': class_assds[2],
        })
