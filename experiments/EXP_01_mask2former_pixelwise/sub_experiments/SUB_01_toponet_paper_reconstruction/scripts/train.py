...
def main(args):
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=min(4, os.cpu_count() or 1), pin_memory=True)
    val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=min(4, os.cpu_count() or 1), pin_memory=True)
    ...
    for epoch in range(args.epoch):
        ...
        if args.batch_size == 1:
            model.apply(freeze_bn)  # Freeze BatchNorm for single-batch stability
        ...
        print("Val IoU:", np.mean(validation_IOU))
        print("Val Dice:", np.mean(mDice))
        print("Val ASSD:", np.mean(mAssd))
        if np.mean(mDice) > best_dice:
            best_dice = np.mean(mDice)
            save_checkpoint_path = os.path.join(args.save_path, "best_model_path.pth")
