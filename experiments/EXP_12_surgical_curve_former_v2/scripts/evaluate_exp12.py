...
def parse_args():
    ...
    p.add_argument("--max_plots",    type=int, default=15)
    return p.parse_args()
        ...
        for b_idx, batch in enumerate(val_loader):
            ...
            for b in range(B):
                ...
                if plots_saved < args.max_plots:
                    ...
                    axes[0].set_title("1. Input Laparoscopic RGB")
                    axes[0].axis("off")
                    ...
                    axes[1].set_title("2. Ground Truth Landmarks")
                    axes[1].axis("off")
                    ...
                    axes[2].set_title("3. CNN Decoder Segmentations")
                    axes[2].axis("off")

                    # 4. Refined Bézier Splines
                    spline_overlay = rgb_img.copy()
                    M_bern = model.hcr.M_bern.cpu().numpy()[:25]  # (25, 6)
                    for m in range(3):
                    ...
                    axes[3].set_title("4. SurgicalCurveFormer v2 Splines")
                    axes[3].axis("off")

                    plt.tight_layout()
                    plot_file = os.path.join(args.output_dir, f"sample_{b_idx:03d}.png")
