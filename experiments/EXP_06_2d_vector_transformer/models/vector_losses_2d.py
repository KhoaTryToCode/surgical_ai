...
class HungarianMatcher2D(nn.Module):
    """
    2D Bipartite Hungarian Matcher for EXP_06 Direct 2D Vector Space Transformer.
    Finds optimal one-to-one assignment between predicted 2D polylines and GT landmarks.
    """
    ...
    def forward(self, pred_cls: torch.Tensor, pred_polylines: torch.Tensor,
                target_cls: torch.Tensor, target_polylines: torch.Tensor, valid_mask: torch.Tensor) -> list:
        """
        pred_cls: (B, N, num_classes+1)
        pred_polylines: (B, N, K, 2) in [0.0, 1.0]^2
        target_cls: (B, N)
        target_polylines: (B, N, K, 2) in [0.0, 1.0]^2
        valid_mask: (B, N)
        Returns: List of (pred_indices, target_indices) tuples per batch element.
        """
        ...
        for b in range(B):
            valid = valid_mask[b]
            gt_c = target_cls[b][valid]
            gt_p = target_poly_f[b][valid]
            num_valid = len(gt_c)
            ...
            # 1. Classification Cost (Softmax probability)
            prob = F.softmax(pred_cls_f[b], dim=-1) # (N, num_classes+1)
            cost_cls = -prob[:, gt_c] # (N, num_valid)

            # 2. Bidirectional 2D L1 Coordinate Cost
            p_pred = pred_poly_f[b] # (N, K, 2)
            
            # Forward drawing order (1..K)
            diff_fwd = p_pred.unsqueeze(1) - gt_p.unsqueeze(0) # (N, num_valid, K, 2)
            cost_fwd = torch.mean(torch.abs(diff_fwd), dim=(-2, -1)) # (N, num_valid)

            # Reverse drawing order (K..1)
...
class Vector2DLossSuite(nn.Module):
    ...
    def _focal_loss(self, inputs: torch.Tensor, targets: torch.Tensor, alpha: float = 0.25, gamma: float = 2.0) -> torch.Tensor:
        """
        Multi-Class Focal Loss for query classification.
    ...
    def forward(self, outputs_cls: list, outputs_polylines: list, outputs_masks: list, targets: dict):
        """
        Calculates Deep Supervision 2D Loss across all decoder layers L.
        """
        target_cls = targets["target_classes"]         # (B, N)
        target_polylines = targets["target_polylines"] # (B, N, K, 2)
        ...
        for l in range(num_layers):
            ...
            pred_poly_l = outputs_polylines[l]       # (B, N, K, 2)
            ...
            for b in range(B):
                ...
                p_matched = pred_poly_l[b, pred_idx].float() # (M, K, 2)
                gt_matched = target_polylines[b, gt_idx].float() # (M, K, 2)

                # 3. Bidirectional 2D L1 Position Loss
        ...
        return total_loss / num_layers, loss_dict
