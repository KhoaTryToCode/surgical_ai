"""
EXP_11: Dataset for SurgicalCurveFormer
=========================================
Extends MacroPatchLandmarkDataset (EXP_10) with:
  1. 5th-order Bézier GT fitting (K=6 ctrl pts, degree=5) instead of cubic (K=4).
  2. ACPI target score map: GT midpoint indicator on f4 grid (H/32 × W/32).
  3. 3-class output: Ridge(0), Silhouette(1), Ligament(2) — 0-indexed to match ACPI.
  4. Render-size GT mask (128×128) for soft rasterizer Dice.

Reuses EXP_10 path resolution, depth map loading, and normalization code.
"""
import os
import json
import glob
...
from models.bezier_ops import fit_bezier_least_squares
...
def resample_polyline_by_arclength(points: np.ndarray, step_size_px: float = 8.0) -> np.ndarray:
    """Resample polyline to uniform arc-length spacing. Copied from shared util."""
    if len(points) < 2:
        return points
...
class SurgicalCurveFormerDataset(Dataset):
    """
    Dataset for EXP_11 SurgicalCurveFormer.

    Output per sample:
        image:              (4, 512, 512)  normalized RGB-D float tensor
        target_masks:       (3, 512, 512)  GT binary stroke masks (Ridge, Sil, Lig)
        target_ctrl_pts:    (3, 6, 2)      GT 5th-order Bézier ctrl pts in [0,1]^2
        active_mask:        (3,)           bool, True if class is present
        acpi_target_score:  (3, H_f, W_f) GT midpoint indicator on ACPI feature map
        target_render_masks:(3, 128, 128)  GT masks at rasterizer resolution
        img_path:           str

    Class indices (0-based):
        0: Ridge (anterior liver ridge)
        1: Silhouette (liver boundary)
        2: Ligament (falciform ligament / vessel)
    """

    # Map annotation labels → 0-based class index
    CLASS_MAP = {
    ...
    def __init__(
        ...
        self.acpi_h = image_size // acpi_stride   # 16 for stride-32
        self.acpi_w = image_size // acpi_stride
        self.render_size = render_size
        self.bezier_degree = bezier_degree  # 5 → K=6 ctrl pts
        ...
        print(
            f"[EXP_11 Dataset] {len(self.image_paths)} images | mode={mode} | "
            f"RGB-D={use_depth} | ACPI grid={self.acpi_h}×{self.acpi_w} | "
    ...
    def _label_to_class(self, label: str) -> int:
        """Map raw annotation label string → 0-based class index. -1 = skip."""
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if img_bgr is None:
            ...
            img_rgb = (img_rgb_hw.astype(np.float32) / 255.0).transpose(2, 0, 1)  # (3, S, S)

        img_norm = (img_rgb - self.mean) / self.std  # (3, S, S)
        img_tensor = torch.from_numpy(img_norm).float()

        # ── Load depth (4th channel) ───────────────────────────────────────
        if self.use_depth:
        ...
        if ann_path is not None:
            try:
                ...
                for shape in data.get("shapes", []):
                    ...
                    # Scale to (S, S) from original annotation resolution
                    pts = raw_pts.copy()
                    ...
                    # ── (A) Render full-res evaluation mask ────────────────
                    pts_pix = np.clip(np.round(dense), 0, S - 1).astype(np.int32)
                    ...
                    # ── (B) Fit 5th-order Bézier control points ────────────
                    # Normalize to [0, 1]^2 for fitting
                    ...
                    # ── (C) ACPI target score: GT midpoint on f4 grid ──────
                    # Midpoint of the curve in normalized coords
                    mid_norm = dense_norm[len(dense_norm) // 2]  # (2,) in [0,1]
                    # Map to f4 grid coordinates (acpi_h × acpi_w)
                    ...
                    # ── (D) Render-size mask (R×R) for Dice loss ──────────
    ...
    def _synthetic_sample(self) -> dict:
        """Synthetic sample for offline smoke testing when no dataset is present."""
        S = self.image_size
        M = self.NUM_CLASSES
        R = self.render_size
        ...
        # Synthetic Ridge curve: control points spanning upper half of image
        ctrl_ridge = np.array([
