"""
Visualization Script for Surgical-BeMapTR v3 (EXP_04b Aux Edge Model).

Extracts predicted polyline curves and auxiliary edge masks, overlays them on surgical images,
...
CLASS_COLORS = {
    1: (0, 255, 0),      # Green for Ridge
    2: (255, 0, 0),      # Blue for Silhouette
    3: (0, 165, 255),    # Orange for Ligament
}


def parse_args():
    parser = argparse.ArgumentParser(description="Visualize Surgical-BeMapTR v3 Predictions")
    ...
    parser.add_argument('--output_dir', type=str, default='/kaggle/working/vis_results_aux')
...
def inspect_and_visualize(ckpt_path, data_path, output_dir, num_samples=5, conf_threshold=0.05, device='cuda'):
    print("=" * 85)
    print(f"🔍 VISUALIZING SURGICAL-BEMAPTR v3 PREDICTIONS FROM: {ckpt_path}")
    print("=" * 85)
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            raw_img = cv2.imread(str(paths[0]))
            ...
            axes[0].set_title(f"Ground Truth ({M} lines)")
            axes[0].axis('off')

            axes[1].imshow(vis_pred)
            axes[1].set_title(f"Surgical-BeMapTR v3 Pred ({len(pred_classes)} lines)")
            axes[1].axis('off')

            axes[2].imshow(aux_vis)
            axes[2].set_title("Aux Edge Feature Heatmap")
            axes[2].axis('off')
