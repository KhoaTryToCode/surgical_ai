"""
EXP_07 — Surgical Bézier Dataset.

Reuses EXP_06's annotation parsing logic, but returns GT polylines (not resampled
to fixed K — keeps original variable-length points for Chamfer loss) plus
rasterized masks for Dice/IoU evaluation.
"""
...
def resample_polyline(points: np.ndarray, num_points: int = 50) -> np.ndarray:
    """
    Resamples a 2D polyline into exactly num_points equidistant points.
...
class SurgicalBezierDataset(Dataset):
    """
    Dataset for EXP_07 SVG Bézier Spline Transformer.

    Returns per sample:
        image:            (3, 1024, 1024) normalized tensor
        target_polylines: (N, K, 2) normalized polyline points in [0, 1]^2
        target_classes:   (N,) class IDs {0=bg, 1=Ridge, 2=Silhouette, 3=Falciform, 4=Gallbladder}
        target_masks:     (N, 1024, 1024) rasterized binary stroke masks (for Dice/IoU only)
        valid_mask:       (N,) boolean indicating active landmarks
    """
    ...
    def __init__(
        ...
        num_polyline_points: int = 50,
        mode: str = "train",
        stroke_thickness: int = 20,
    ):
        super().__init__()
        self.dataset_dir = dataset_dir
        self.num_instances = num_instances
        self.num_polyline_points = num_polyline_points
        self.mode = mode
        ...
        # Index all JSON annotation files
        self.json_index = {}
        ...
        found_anns = sum(1 for p in self.image_paths if self._find_annotation(p) is not None)
        print(f"📊 [{mode.upper()} DATASET] {len(self.image_paths)} Images | {found_anns} Annotated JSONs located.")

    def __len__(self) -> int:
        return len(self.image_paths)

    def _find_annotation(self, img_path: str):
        ...
        candidates = [
            img_path.replace("images", d).replace(".jpg", ".json").replace(".png", ".json")
            for d in ["labels", "annotations", "label"]
        ] + [
    ...
    def _parse_class(self, label: str) -> int:
        label = label.lower().replace("-", "_").replace(" ", "_")
        if label.startswith("r") or "ridge" in label:
            return 1
        if label.startswith("s") or "silhouette" in label:
            return 2
        if label.startswith("l") or "ligament" in label or "falciform" in label or "vessel" in label:
            return 3
        if "gall" in label:
            return 4
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        K = self.num_polyline_points
        target_polylines = np.zeros((N, K, 2), dtype=np.float32)
        target_classes = np.zeros((N,), dtype=np.int64)
        target_masks = np.zeros((N, 1024, 1024), dtype=np.float32)
        ...
        if ann_path is not None and os.path.exists(ann_path):
            try:
                ...
                for shape in shapes:
                    ...
                    cls_id = self._parse_class(label)
                    ...
                    # Resample to K equidistant points
                    poly = resample_polyline(pts_norm, num_points=K)

                    # Rasterize binary mask for Dice/IoU evaluation
                    mask_canvas = np.zeros((1024, 1024), dtype=np.uint8)
                    pts_pix = (poly * 1024.0).astype(np.int32).reshape((-1, 1, 2))
                    cv2.polylines(mask_canvas, [pts_pix], isClosed=False, color=1, thickness=self.stroke_thickness)

                    target_polylines[inst_idx] = poly
